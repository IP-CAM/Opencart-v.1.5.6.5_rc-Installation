	...
	private function updateOrderWithConfirmedData($order_id, $order, $user) {
		...
		foreach ($order->txn as $txn) {
			...
			if ($this->tax_type == 1) {
				...
				$totalTax += (float)$txn->item->tax->total;
				$totalNet += $price * $qty;
			} else {
				//use the store pre-set tax-rate for everything
		...
		if ($this->tax_type == 1) {
			$discountNet    = (float)$order->order->discount;
			$shippingNet    = (float)$order->shipping->cost;

			$tax = number_format($totalTax, 4, '.', '');
		} else {
			$discountNet    = (float)$order->order->discount / $this->tax;
			$discountTax    = (float)$order->order->discount - $discountNet;
			$shippingNet    = (float)$order->shipping->cost / $this->tax;
			$shippingTax    = (float)$order->shipping->cost - $shippingNet;

			$tax = number_format($shippingTax + $totalTax + $discountTax, 4, '.', '');
		}

		$totals = number_format((float)$totalNet + (float)$shippingNet + (float)$tax + (float)$discountNet, 4, '.', '');
		...
		$data['totals'][0] = array(
			...
			'value'      => number_format((float)$totalNet, 4, '.', ''),
			'sort_order' => '1'
		);
		...
		$data['totals'][1] = array(
			...
			'value'      => number_format((float)$shippingNet, 4, '.', ''),
			'sort_order' => '3'
		);
		...
		$data['totals'][2] = array(
			...
			'value'      => number_format((float)$discountNet, 4, '.', ''),
			'sort_order' => '4'
		);
