			...
			if (!in_array($item['ItemID'], $current)) {
				...
				$this->db->query("
					INSERT INTO `" . DB_PREFIX . "product` SET
						...
						`tax_class_id`          = '9',
						`location`              = '" . $this->db->escape($item['note'] ?? '') . "',
						`mpn`              		= '" . $this->db->escape($item['advanced']['brand']['mpn'] ?? '') . "',
						`sku`              		= '" . $this->db->escape($item['SKU'] ?? '') . "',
						`model`              	= '" . $this->db->escape($item['SKU'] ?? '') . "',
						`isbn`              	= '" . $this->db->escape($item['advanced']['isbn'] ?? '') . "',
						`ean`              		= '" . $this->db->escape($item['advanced']['ean'] ?? '') . "',
						`upc`              		= '" . $this->db->escape($item['advanced']['upc'] ?? '') . "',
						`weight`       			= '" . (float)$weight . "',
				...
				if (is_array($item['pictures'])) {
					foreach ($item['pictures'] as $img) {
						if (!empty($img)) {
							$name = mt_rand(500000, 1000000000);
