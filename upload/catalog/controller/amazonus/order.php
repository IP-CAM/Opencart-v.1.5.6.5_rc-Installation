	...
	public function index() {
		...
		foreach ($orderXml->Items->Item as $item) {
			...
			if ($taxTotal == 0 && $this->config->get('openbay_amazonus_order_tax') > 0) {
				$taxTotal = (float)$item->Totals->Price * ($this->config->get('openbay_amazonus_order_tax') / 100) / (1 + $this->config->get('openbay_amazonus_order_tax') / 100);
			}
			...
			if ($shippingTax == 0 && $this->config->get('openbay_amazonus_order_tax') > 0) {
				$shippingTax = (float)$item->Totals->Shipping * ($this->config->get('openbay_amazonus_order_tax') / 100) / (1 + $this->config->get('openbay_amazonus_order_tax') / 100);
			}
			...
			if ($itemGiftWrapTax == 0 && $this->config->get('openbay_amazonus_order_tax') > 0) {
				$itemGiftWrapTax = (float)$item->Totals->GiftWrap * ($this->config->get('openbay_amazonus_order_tax') / 100) / (1 + $this->config->get('openbay_amazonus_order_tax') / 100);
			}
