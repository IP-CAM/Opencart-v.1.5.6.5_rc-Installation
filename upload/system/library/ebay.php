...
final class Ebay {
	...
	public function isEbayOrder($id) {
		...
		if ($qry->num_rows) {
			...
			return str_replace(array('[eBay Import:', ']'), '', $qry->row['comment']);
		} else {
		...
		if ($item_id != false) {
			...
			if ($this->openbay->addonLoad('openstock') && (isset($data['has_option']) && $data['has_option'] == 1)) {
				...
				if ($stock == true) {
					...
					return $this->call('item/reviseStockVariants', $varData);
				} else {
		...
		if ($ebay_id != false) {
			...
			if (!empty($item_txn_array)) {
				...
				if ($status_id == $this->config->get('EBAY_DEF_SHIPPED_ID')) {
					...
					foreach ($item_txn_array as $item) {
						$tmp = simplexml_load_string($this->eBayShippingStatus($item['item'], $item['txn'], true, ($data['tracking_no'] ?? ''), ($data['carrier_id'] ?? '')));
					}
					$qry = $this->db->query("UPDATE `" . DB_PREFIX . "ebay_order` SET `carrier_id` = '" . $this->db->escape(($data['carrier_id'] ?? '')) . "', `tracking_no` = '" . $this->db->escape(($data['tracking_no'] ?? '')) . "' WHERE `order_id` = '" . (int)$order_id . "' LIMIT 1");
				}
